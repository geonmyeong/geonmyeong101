...
{
    public int hp = 180;                    //변수 hp 선언 180 값 입력 (public은 인스팩터 창에서 보이기 하기 위하여 사용)
    public Text hpText;
    public Text hpStatus;                               //Hp 숫자 표시 UI
    void Update()
    {
        hpStatus.text = hp.ToString();              //hp 숫자 표시 toSrting 은 정수를 문자열로 변환

        if (Input.GetMouseButtonDown(0))             // 0은 왼쪽 클릭
        {
            hp += 10;
        }
        if (Input.GetMouseButtonDown(1))             // 1은 왼쪽 클릭
        {
            hp -= 10;
        }
